...
class _Transformer(aas_types.AbstractTransformer[Iterator[Error]]):
    ...
    def transform_administrative_information(
        ...
        if not (
            not (that.revision is not None) or matches_revision_type(that.revision)
        ):
            yield Error("Revision shall match the revision pattern")

        if not (not (that.version is not None) or matches_version_type(that.version)):
            yield Error("Version shall match the version pattern")

        if not (
            not (that.revision is not None)
            or ((len(that.revision) > 0 and len(that.revision) <= 4))
        ):
            yield Error(
